...
class RiskMonitor:
    ...
    def _calculate_mdd(self, portfolio_returns: pd.Series) -> float:
        ...
        # Log and handle extreme negative returns to prevent calculation errors.
        min_return = portfolio_returns.min()
        if min_return <= -0.999: # Check for returns close to -100%
            problematic_date = portfolio_returns.idxmin()
            logger.warning(
                f"MDD calculation encountered an extreme negative return of {min_return:.2%} "
                f"on {problematic_date.date()}. This can cause inaccurate MDD. "
                "Capping at -99.99% for this calculation."
            )
            logger.debug(f"Full returns series around problematic date:\n{portfolio_returns.loc[problematic_date - pd.Timedelta(days=3):problematic_date + pd.Timedelta(days=3)]}")

        # Clip values to be slightly greater than -1 for numerical stability.
        safe_returns = portfolio_returns.clip(lower=-0.9999)
        cumulative_returns = (1 + safe_returns).cumprod()
        peak = cumulative_returns.expanding(min_periods=1).max()
        drawdown = (cumulative_returns / peak) - 1
        
        # If drawdown is empty or all NaN, return 0
    ...
    def run_analysis(
        ...
        analysis_rets = returns_df.loc[:, portfolio_tickers]
        analysis_weights = weights.loc[portfolio_tickers]
    ...
    def generate_report_text(self, analysis_results: Dict) -> str:
        ...
        for metric, data in analysis_results.items():
            ...
            line = f"  - {metric.replace('_', ' ').title():<20}: "
            if 'hhi' in metric:
                line += f"{value:8.4f} "
            else:
                line += f"{value:8.2%} "
            ...
            if 'mdd' in metric:
                line += f"(Threshold: < {abs(threshold):.2%}, Status: {status})"
            elif 'hhi' in metric:
    ...
    def save_report(self, report_text: str, output_dir: str):
        ...
        # Add timestamp to filename to ensure uniqueness for each run
        file_path = path / f"risk_report_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.txt"
